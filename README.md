...
- Скачайте [последний релиз](https://github.com/ExxDreamerCode/Ember/releases/tag/V0.9.3)
...
Движок поддерживает Polyglot-формат дебютных книг (.bin). При запуске он автоматически ищет `book.bin` рядом с исполняемым файлом. Если книга найдена — ходы в дебюте берутся из неё мгновенно.
