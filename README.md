...
- **Native Ember** — Ember's own `.nnue` format. A half-based feature transformer with
...
- **Compact (ECN1)** — Ember's packed compact variant of the native format (magic
  `ECN1`, generated by `training/compact_nnue.py`). It shrinks the feature transformer
  to a dense basis plus a correction map, so it is not an independent architecture, just
  a storage layout of the native network. The embedded network is stored in this form
  and loads through the `NNUE` option as `<embedded>`.
...
- **External networks** — a container format for foreign architectures with a fixed
  feature layout (a half-transformer with PSQ and threat features followed by several
  `Affine` stacks), whose dimensions are validated at load. Shown as
  `Loaded external net <path> (arch hash=... desc="..." ...)`.
