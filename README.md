...
2. Откройте `battle.toml` в Notepad и задайте список игр. Конфигурация из ZIP
   уже готова к одному casual 3+2 матчу с первым доступным ботом из пула.
3. Запустите `Verify.cmd`.
4. Запустите `Run Battle.cmd`, проверьте напечатанный план, введите `YES`, затем
   введите токен Lichess в скрытом приглашении.
...
Пул по умолчанию: `Lynx_BOT`, `pawn_git`, `simbelmyne-bot`, `CubixChess` и
`bot_adario`. Их blitz-рейтинг находится примерно в требуемом диапазоне, но
рейтинг и доступность внешних community-ботов со временем меняются. Runner
одним запросом проверяет online/busy-статус всего пула, вызывает первого
свободного, а при отказе пробует следующего. Сообщение Lichess с точным
`please wait until ...` превращается в cooldown именно для этого бота.
...
Игры создаются как прямые вызовы строго по очереди. В каждой `[[games]]` можно
...
это явно задаётся `opponent_wait_timeout_seconds = 0`. Все попытки, причины
отказов, cooldown, время ожидания и выбранный соперник записываются. Ошибки
токена и его permissions остаются фатальными. `scoring` и `tags` — метки
для последующего анализа; casual/rated задаётся полем `mode`. Matchmaking и
Syzygy отключены. Запуск остаётся в видимом окне и не создаёт сервис, autostart,
scheduled task и не меняет power/sleep, registry, PATH или firewall Windows.

Мониторинг игры использует отдельные безопасные GET-запросы к Lichess. Временные
обрывы соединения и HTTP 5xx повторяются с backoff и не останавливают отдельный
процесс `lichess-bot`, который ведёт уже начатую игру. Если дополнительный PGN-
экспорт после партии временно недоступен, JSON и PGN самого `lichess-bot`
сохраняются, а run завершается с предупреждением вместо падения.
