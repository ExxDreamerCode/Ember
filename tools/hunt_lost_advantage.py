...
from advantage_fixture import validate_fixture_case, write_fixture
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture-output",
        default=None,
        help="fixture destination (default: inside this run's result directory)",
    )
    parser.add_argument(
        "--overwrite-fixture",
        action="store_true",
        help="allow replacing an existing explicit fixture destination",
    )
...
def resolve_fixture_output(args: argparse.Namespace, run_root: Path) -> Path:
    output = (
        Path(args.fixture_output)
        if args.fixture_output is not None
        else run_root / "advantage_preservation.tsv"
    )
    if output.exists() and not args.overwrite_fixture:
        raise FileExistsError(
            f"fixture output already exists: {output}; pass --overwrite-fixture"
        )
    return output


def fixture_rejection(case: LostAdvantageCase) -> str | None:
    try:
...
def main(argv: list[str]) -> int:
    ...
    try:
        fixture_output = resolve_fixture_output(args, run_root)
    ...
    try:
        for game_index in range(1, args.max_source_games + 1):
            ...
            if replay is None:
                continue
            rejection = fixture_rejection(replay)
            if rejection is not None:
                rejected_cases.append(
                    {"case": asdict(replay), "reason": rejection}
                )
                continue
    ...
    (run_root / "rejected-cases.json").write_text(
        json.dumps(rejected_cases, indent=2),
        encoding="utf-8",
    )
    write_summary(run_root / "summary.md", cases, args)
    write_fixture(fixture_output, cases)
    print(f"wrote {len(cases)} cases to {run_root}", flush=True)
    print(f"fixture: {fixture_output}", flush=True)
    if len(cases) < args.cases:
