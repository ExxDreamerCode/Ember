...
def write_fixture(path: Path, cases: list[LostAdvantageCase]) -> None:
    lines = [
        ...
        "# All collected cases are active rows: they count toward the solve-rate",
        "# meter but never block the gate by themselves. Verify expected moves",
        "# honestly with a strong reference engine before tuning to a bucket.",
        "",
        FIXTURE_HEADER,
    ]
    for case in cases:
        lines.extend(["", fixture_comment(case), fixture_row(case)])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
