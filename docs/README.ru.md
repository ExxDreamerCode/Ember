...
- **Собственная сеть Ember** — родной формат `.nnue`. Фича-экстрактор на основе
...
- **Компактный формат (ECN1)** — сжатый вариант собственной сети Ember (магическое
  число `ECN1`, создаётся скриптом `training/compact_nnue.py`). Он сокращает
...
- **Внешние сети** — формат-контейнер для сторонних архитектур с фиксированной
  ...
  `Loaded external net <путь> (arch hash=... desc="..." ...)`.
