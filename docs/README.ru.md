...
Когда движок выбирает ход из книги, он сообщает выбор с нулевой поисковой
телеметрией и меткой источника:
...
- **Компактный формат (ECN1)** — сжатый вариант собственной сети V1 Ember (магическое
  число `ECN1`, создаётся скриптом `training/ru/v1/compact_nnue_v1.py`). Он сокращает
  ...
  загружаются внешние файлы `ECN1`; сама встроенная сеть теперь находится в формате
  Ember V2.
...
небольшому встроенному набору позиций с отчётом по узлам, NPS и воспроизводимой
сигнатурой количества узлов:

```
bench              # весь набор на глубине 10
bench 12           # весь набор на глубине 12
bench depth 12 positions 4   # первые 4 позиции набора на глубине 12
```

Она нужна для быстрых сравнений внутри процесса и smoke-тестов релизных сборок; полный
замер формы поиска описан в [quality-assessment.md](quality-assessment.md).
